...
class NMT(nn.Module):
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        while len(completed_hypotheses) < beam_size and t < args.decode_max_time_step:
            ...
            live_hyp_ids = []
            new_hyp_scores = []
            print(prev_hyp_ids.cpu().data, tgt_vocab_size, top_new_hyp_scores, hyp_scores)
            for prev_hyp_id, word_id, new_hyp_score in zip(prev_hyp_ids.cpu().data, word_ids.cpu().data, top_new_hyp_scores.cpu().data):
#                if len(hypotheses) < prev_hyp_id:
#                    print(prev_hyp_id, len(hypotheses))
#                    continue
            ...
            for i, hyp in enumerate(completed_hypotheses):
                ...
                for w in hyp:
                    ...
                    if torch.is_tensor(w):
                        lis.append(self.vocab.tgt.id2word[w.item()])
                    else:
                        lis.append(self.vocab.tgt.id2word[w])

                # completed_hypotheses_words.append(lis)

                completed_hypotheses[i] = lis
    ...
    while True:
        ...
        for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
            ...
            if train_iter % args.valid_niter == 0:
                ...
                dev_ppl = np.exp(dev_loss.cpu().numpy())
